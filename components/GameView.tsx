    ...
    <div style={{
      ...
      <div style={{ 
        ...
        {turnOrderCharacters.map((char, index) => {
          ...
          // En mode JcJ, afficher "Équipe Bleue/Rouge" au lieu de "Vous/Ennemi"
          const teamLabel = gameMode === 'pvp' 
            ? (isPlayer ? '(Équipe Bleue)' : '(Équipe Rouge)') 
        ...
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.85)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 100 }}>
          <div style={{ textAlign: 'center', padding: '40px', borderRadius: '20px', background: 'rgba(30,30,50,0.95)', border: '1px solid rgba(255,255,255,0.1)' }}>
            ...
            <h2 style={{ fontSize: '32px', fontWeight: 'bold', marginBottom: '8px', color: gameState.winner === 'player' ? (gameMode === 'pvp' ? '#3b82f6' : '#fbbf24') : '#ef4444' }}>
              {gameMode === 'pvp'
                ? (gameState.winner === 'player' ? 'ÉQUIPE BLEUE GAGNE !' : 'ÉQUIPE ROUGE GAGNE !')
                : (gameState.winner === 'player' ? 'VICTOIRE !' : 'DÉFAITE')}
            </h2>
            <p style={{ color: '#9ca3af', marginBottom: '24px' }}>
              {gameMode === 'pvp' 
                ? (gameState.winner === 'player' ? 'L\'équipe bleue remporte la partie !' : 'L\'équipe rouge remporte la partie !')
